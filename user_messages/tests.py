...
from user_messages.utils import format_subject, format_quote
...
class FeedbackTestCase (TestCase):
    """
    Tests the feedback message sending.-
    """
    ...
    def setUp(self):
        """
        Create a user and a client used during testing.-
        """
        self.user1 = User.objects.create_user (**self.T_USER_DATA[0])
        self.cli = Client ( )
        self.cli.login (username=self.T_USER_DATA[0]['username'],
                        password=self.T_USER_DATA[0]['password'])

    def testUserFeedback (self):
        """
        Test a feedback message sent by a registered user.-
        """
        response = self.cli.get (reverse ('user_messages.views.feedback'))
        self.assertEquals (response.status_code, 200)
        self.assertEquals (response.template[0].name, 'user_messages/feedback.html')
        form = response.context[-1]['form']
        self.assertEquals (form.fields['recipient'].initial[0], 
                           User.objects.get (username='root'))
        self.assertTrue (u'Feedback' in form.fields['subject'].initial)
        
        response = self.cli.post (reverse ('user_messages.views.feedback'),
                                  {'recipient': self.T_USER_DATA[1]['username'],
                                   'subject': self.T_MESSAGE_DATA[0]['subject'],
                                   'body': self.T_MESSAGE_DATA[0]['body']})
        self.assertEquals (response.context[-1]['success_url'], reverse ('ht.views.home'))
...
class IntegrationTestCase(TestCase):
    ...
    def testInboxEmpty(self):
        ...
        response = self.c.get(reverse('user_messages_inbox'))
        self.assertEquals(response.status_code, 200)
        self.assertEquals(response.template[0].name, 'user_messages/inbox.html')
        self.assertEquals(len(response.context['message_list']), 0)
    
    def testOutboxEmpty(self):
        """ request the empty outbox """
        response = self.c.get(reverse('user_messages_outbox'))
        self.assertEquals(response.status_code, 200)
        self.assertEquals(response.template[0].name, 'user_messages/outbox.html')
        self.assertEquals(len(response.context['message_list']), 0)

    def testTrashEmpty(self):
        """ request the empty trash """
        response = self.c.get(reverse('user_messages_trash'))
        self.assertEquals(response.status_code, 200)
        self.assertEquals(response.template[0].name, 'user_messages/trash.html')
        self.assertEquals(len(response.context['message_list']), 0)

    def testCompose(self):
        """ compose a message step by step """
        response = self.c.get(reverse('user_messages_compose_empty'))
        self.assertEquals(response.status_code, 200)
        self.assertEquals(response.template[0].name, 'user_messages/compose.html')
        response = self.c.post(reverse('user_messages_compose_empty'),
            {'recipient': self.T_USER_DATA[1]['username'],
             'subject': self.T_MESSAGE_DATA[0]['subject'],
             'body': self.T_MESSAGE_DATA[0]['body']})
        # successful sending should redirect to inbox
        self.assertEquals(response.status_code, 302)
        self.assertEquals(response['Location'], "http://testserver%s"%reverse('user_messages_inbox'))
    ...
    def testReply(self):
        ...
        response = self.c.get(reverse('user_messages_reply', 
